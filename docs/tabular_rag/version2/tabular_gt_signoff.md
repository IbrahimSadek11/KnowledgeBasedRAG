...
| Q62 | **N/A in version2** — schema no longer distinguishes 'entered but no ranked result' from 'never entered' (`event_entries` removed). Original gold SQL kept for reference only: `SELECT DISTINCT e.event_id, e.location, e.category, e.event_date, e.discipline FROM events e LEFT JOIN event_entries ee ON e.event_id = ee.event_id LEFT JOIN event_participations ep ON e.event_id = ep.event_id WHERE ep.participation_id IS NULL;` |
| Q63 | `SELECT event_id, COUNT(*) AS cnt FROM event_participations GROUP BY event_id ORDER BY cnt DESC LIMIT 1;` |
| Q65 | **N/A in version2** — same collapsed distinction as Q62. Original: `SELECT COUNT(DISTINCT e.horse_id) AS unranked, (SELECT COUNT(DISTINCT horse_id) FROM event_entries) AS total FROM event_entries e LEFT JOIN event_participations p ON e.horse_id = p.horse_id AND e.event_id = p.event_id WHERE p.participation_id IS NULL;` |
...
| Q67 | **N/A in version2** — same collapsed distinction as Q62. Original: `SELECT ee.event_id, COUNT(DISTINCT ee.horse_id) - COALESCE((SELECT COUNT(*) FROM event_participations ep WHERE ep.event_id = ee.event_id), 0) AS gap FROM event_entries ee GROUP BY ee.event_id ORDER BY gap DESC LIMIT 1;` |
...
| Q86 | `SELECT t.horse_id, COUNT(*) AS stage_count, (SELECT COUNT(DISTINCT event_id) FROM event_participations ep WHERE ep.horse_id = t.horse_id) AS competition_count FROM trainings t GROUP BY t.horse_id;` |
| Q90 | `SELECT DISTINCT training_id, horse_id, volume FROM trainings WHERE stage_type = 'TransitionStage';` |
| Q91 | **N/A in version2** — same collapsed distinction as Q62. Original: `SELECT ee.event_id, ep.rank FROM event_entries ee LEFT JOIN event_participations ep ON ee.event_id = ep.event_id AND ee.horse_id = ep.horse_id WHERE ee.horse_id = 'Horse_Auroch';` |
| Q93 | `SELECT t.horse_id, COUNT(*) AS stage_count, (SELECT COUNT(DISTINCT event_id) FROM event_participations ep WHERE ep.horse_id = t.horse_id) AS competition_count FROM trainings t GROUP BY t.horse_id;` |
...
| Q98 | **N/A in version2** — same collapsed distinction as Q62. Original: `SELECT COUNT(*) FROM event_entries ee LEFT JOIN event_participations ep ON ee.horse_id = ep.horse_id AND ee.event_id = ep.event_id WHERE ep.participation_id IS NULL;` |
...
**(b) Original gold SQL (version1 / live; not executed in version2):**  
```sql
SELECT ee.event_id,
...
**version2 note:** N/A — `event_entries` removed; this query tested the
'entered but no ranked result' distinction that is collapsed in version2.
(Historical V9 correction notes for HAVING gap > 0 are retained only in
version1 / live docs history — not applicable here.)
...
### Q4 — CLEAN (version2: rewritten to event_participations)
...
**(c) Proposed correction (version1 / live):**  
`SELECT event_id FROM event_entries WHERE horse_id = 'Horse_Dakota';`

**(c′) version2 gold SQL:**  
`SELECT DISTINCT event_id FROM event_participations WHERE horse_id = 'Horse_Dakota';`
...
### Q17 — CLEAN (version2: rewritten to event_participations)
...
**(c′) version2 gold SQL:**  
`SELECT DISTINCT rider_id FROM event_participations WHERE horse_id = 'Horse_Dakota';`

**(d) Live:** `Rider_Emma`, `Rider_Manon`

---

### Q18 — CLEAN (version2: rewritten to event_participations)
...
**(c′) version2 gold SQL:**  
`SELECT DISTINCT rider_id FROM event_participations WHERE horse_id = 'Horse_Naya';`

**(d) Live:** `Rider_Leo`

---

### Q43–Q45 — CLEAN (version2: rewritten to event_participations)

| Q | version2 gold SQL |
|---|---|
| Q43 | `SELECT rider_id, COUNT(DISTINCT horse_id) AS horse_count FROM event_participations GROUP BY rider_id;` |
| Q44 | `SELECT horse_id FROM event_participations GROUP BY horse_id HAVING COUNT(DISTINCT rider_id) > 1;` |
| Q45 | `SELECT rider_id, COUNT(DISTINCT horse_id) AS horse_count FROM event_participations GROUP BY rider_id ORDER BY horse_count DESC LIMIT 1;` |
