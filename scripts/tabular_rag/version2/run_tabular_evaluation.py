"""
Run Tabular RAG Evaluation — VERSION2 snapshot (12-table schema).

Uses backend/tabular_rag/version2/tabular_chain.py and
data/tabular_rag/version2/tabular.db. Gold SQL from this folder's
...
# ══════════════════════════════════════════════════════════════
# Configuration (VERSION2 paths — not live)
# ══════════════════════════════════════════════════════════════

SCRIPT_DIR = os.path.dirname(os.path.abspath(__file__))
# .../scripts/tabular_rag/version2 -> repo root = 4 dirname levels
PROJECT_ROOT = os.path.abspath(
    os.path.join(SCRIPT_DIR, "..", "..", "..")
)
V2_BACKEND = os.path.join(PROJECT_ROOT, "backend", "tabular_rag", "version2")
...
# Load version2 sql_validator + tabular_chain (override live package modules)
import backend.tabular_rag as _tr_pkg  # noqa: E402

_sv = _load_as(
    "backend.tabular_rag.sql_validator",
    os.path.join(V2_BACKEND, "sql_validator.py"),
)
_tr_pkg.sql_validator = _sv
_chain = _load_as(
    "backend.tabular_rag.tabular_chain",
    os.path.join(V2_BACKEND, "tabular_chain.py"),
)
_chain.DB_PATH = DB_PATH
_tr_pkg.tabular_chain = _chain

from backend.tabular_rag.tabular_chain import answer_question  # noqa: E402
...
from gold_queries import GOLD_QUERIES, EX_NOT_APPLICABLE, compare_execution  # noqa: E402

print("[version2 eval config]")
print(f"  PROJECT_ROOT      = {PROJECT_ROOT}")
print(f"  V2_BACKEND        = {V2_BACKEND}")
print(f"  chain module file = {_chain.__file__}")
print(f"  chain.DB_PATH     = {_chain.DB_PATH}")
print(f"  DB_PATH (EX)      = {DB_PATH}")
print(f"  RESULTS_DIR       = {RESULTS_DIR}")
print(f"  gold_queries file = {os.path.join(SCRIPT_DIR, 'gold_queries.py')}")
print(f"  GOLD_QUERIES Q4   = {GOLD_QUERIES.get('Q4')}")
print(f"  EX_NOT_APPLICABLE has Q62? {('Q62' in EX_NOT_APPLICABLE)}")
