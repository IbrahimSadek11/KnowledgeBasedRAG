...
# Version2 snapshot: write under data/tabular_rag/version2/ (not live).
# __file__ = .../backend/tabular_rag/version2/tabular_etl.py -> 4 dirname to repo root
_ETL_FILE = os.path.abspath(__file__)
PROJECT_ROOT = os.path.dirname(os.path.dirname(os.path.dirname(os.path.dirname(_ETL_FILE))))
if PROJECT_ROOT not in sys.path:
    sys.path.insert(0, PROJECT_ROOT)

from backend.config import NEO4J_URI, NEO4J_USER, NEO4J_PASSWORD, NEO4J_DATABASE

DATA_DIR = os.path.join(PROJECT_ROOT, "data", "tabular_rag", "version2")
...
RETURN p.id AS person_id, p.hasName AS name, labels(p) AS labels
"""
...
def fetch_from_neo4j():
    ...
    try:
        with driver.session(database=NEO4J_DATABASE) as session:
            ...
            people = [
                (rec["person_id"], pick_actor_role(rec["labels"]), rec["name"])
...
def build_sqlite(
    ...
    riders = [(person_id,) for person_id, role, _name in people if role == "Rider"]
    veterinarians = [
        (person_id,) for person_id, role, _name in people if role == "Veterinarian"
    ]
    caretakers = [
        (person_id,) for person_id, role, _name in people if role == "Caretaker"
    ]
    ...
    try:
        ...
        cur.executemany(
            "INSERT INTO people (person_id, role, name) VALUES (?, ?, ?)",
            people,
        )
        ...
        # 11. event_participations (official results)
        cur.execute(
            """
        ...
        print(f"\n(a) seasons: {season_count} (expected 1)")
...
def check_v9_staleness(db_path=DB_PATH, report_path=STALENESS_REPORT_PATH):
    ...
    return not mismatches and null_ranks == 0 and bad_actors == 0
